import { PostDataRequestArgs, getDataRequestId } from "./models/data-request.js";
import { getLatestProxyAddress } from "./services/get-latetst-proxy-address.js";
import { Signer } from "./signer.js";

export async function postDataRequest(callParams: PostDataRequestArgs, signer: Signer) {
  ...
  const client = signer.client.unwrapOrElse(() => { throw new Error('No client on signer available') });
  const account = signer.account.unwrapOrElse(() => { throw new Error('No address on signer available') });
