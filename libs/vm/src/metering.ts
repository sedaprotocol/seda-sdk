...
export class GasMeter {
	...
	applyGasCost(type: CallType, bytesLength: number) {
		let gasCost: bigint;

		switch (type) {
			case CallType.ExecutionResult:
				gasCost = GAS_PER_BYTE_EXECUTION_RESULT * BigInt(bytesLength);
				break;
			case CallType.HttpFetchRequest:
				gasCost = GAS_HTTP_FETCH_BASE + GAS_PER_BYTE * BigInt(bytesLength);
				break;
			case CallType.HttpFetchResponse:
				gasCost = GAS_PER_BYTE * BigInt(bytesLength);
				break;
			case CallType.Bn254Verify:
				gasCost = GAS_BN254_VERIFY_BASE + GAS_PER_BYTE * BigInt(bytesLength);
				break;
			case CallType.ProxyHttpFetchRequest:
				gasCost = GAS_PROXY_HTTP_FETCH_BASE + GAS_PER_BYTE * BigInt(bytesLength);
				break;
			case CallType.Secp256k1Verify:
				gasCost =
					GAS_SECP256K1_BASE + GAS_KECCAK256_BASE + GAS_PER_BYTE * BigInt(bytesLength);
				break;
			case CallType.Keccak256:
				gasCost = GAS_KECCAK256_BASE + GAS_PER_BYTE * BigInt(bytesLength);
				break;
			default:
				throw new Error("Unknown call type");
		}

		this.applyOpcodeGas(gasCost);
	}
}
