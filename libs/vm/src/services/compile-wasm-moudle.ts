...
export async function createWasmModule(binary: Uint8Array | WebAssembly.Module | number[], options?: CacheOptions): Promise<WebAssembly.Module> {
    if (binary instanceof WebAssembly.Module) {
        return binary;
    }

    const binaryArray = binary instanceof Uint8Array
        ? binary
