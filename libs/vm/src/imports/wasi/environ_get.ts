...
function getEnvBytesLength(env: Record<string, string>) {
    return Object.entries(env).reduce((acc, [key, value]) => acc + key.length + value.length + 2, 0);
}
...
export function environ_get(
    ...
    gasMeter.applyGasCost(CallType.EnvironGet, BigInt(getEnvBytesLength(vmCallData.envs)));
...
export function environ_sizes_get(
    ...
    gasMeter.applyGasCost(CallType.EnvironSizesGet, BigInt(getEnvBytesLength(vmCallData.envs)));
	const func = extractFunctionFromImportValue(importValue);
