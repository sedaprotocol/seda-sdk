...
export default class DataRequestVmAdapter implements VmAdapter {
	...
	async httpFetch(
		...
		// Set up local and global timeouts.
		const abortController = new AbortController();

		const localTimeout = action.options.timeout_ms ?? 2_000;
		const localTimeoutMsg = `HTTP fetch time limit exceeded (${localTimeout}ms)`;
		...
		try {
			...
			if (hasLocalTimeout) {
				throw new VmError(localTimeoutMsg, {
					type: VmErrorType.HttpFetchTimeout,
				});
			}
